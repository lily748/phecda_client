...
import { copyFileSync, createReadStream, createWriteStream, exists, existsSync, mkdir, mkdirSync, readdir, readdirSync, readFileSync, renameSync, rmdirSync, stat, statSync, unlinkSync, writeFileSync } from "fs";
...
class _Tools {
    ...
    zipDirc(dir: string, out: string, folder: string) {
        let JSZIP = require("jszip");
        let jszip = new JSZIP();
        this.zipDir(normalize(dir), jszip.folder(folder));

        let packZipRootPath = path.join(Editor.Project.path, "build/Publish_Package")
        packZipRootPath = normalize(packZipRootPath);

        if (!existsSync(packZipRootPath)) {
            this.mkdirSync(packZipRootPath);
        }
        let packVersionZipPath = path.join(packZipRootPath, folder + ".zip");
        return new Promise((resolve, reject) => {
            let renamePath = path.join(packZipRootPath, out + ".zip")
            jszip.generateNodeStream({
                type: "nodebuffer",
                streamFiles: !0
            }).pipe(createWriteStream(packVersionZipPath)).on("finish", () => {
                console.log("[打包] 打包成功: " + renamePath)
                renameSync(packVersionZipPath, renamePath);
                resolve(0)
            }).on("error", (e: Error) => {
    ...
    zipVersions(config: ZipVersionsConfig) {
        ...
        for (let index = 0; index < bundles.length; index++) {
            ...
            let packVersionZipPath = path.join(packZipRootPath, packZipName);
            let jszip = new JSZIP();
            let fullPath = path.join(config.buildDir, `assets/${element.bundle}`);
    ...
    copyFile(_source: string, _dest: string) {
    ...
    getRelative(source: string, dest: string) {
        var relativePath = path.relative(source, dest);
