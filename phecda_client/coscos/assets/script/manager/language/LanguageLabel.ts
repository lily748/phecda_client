...
export class LanguageLabel extends Component {
    ...
    @property({ type: LangLabelParamsItem, displayName: "params" })
    ...
    get string(): string {
        let _string = LanguageManager.instance.getLangByID(this._dataID, this.bundleName)
        ...
        if (!_string) {
            console.warn(this.bundleName + "[LanguageLabel] 未找到语言标识，使用dataID替换")
    ...
    private _initFontSize: number = 0
    ...
    onLoad() {
        ...
        if (this.getComponent(RichText)) {
            this._initFontSize = this.getComponent(RichText)!.fontSize
        }

        if (this.getComponent(Label)) {
            this._initFontSize = this.getComponent(Label)!.fontSize
        }
    }
